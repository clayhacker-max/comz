...
class fucking:
	...
	def pantad(list,saving):
		...
		dat = {
					"garis":"#-------------------------------------------------#\n",
					"github":"# github  : https://github.com/clayhacker-max\n",
					"from":"# from "+meki[0]+"\n",
					"local":"# "+meki[1]+" : "+meki[4]+"\n",
					"key":"# key : Asep-"+key+"\n",
					"date":"# date : "+is_time+"\n"
				}
		dats += dat["garis"]+"# Obfuscate By Mr.Gaming Thanks To Black Coder Crush\n"+dat["github"]+dat["from"]+dat["local"]+dat["key"]+dat["date"]+dat["garis"]
		if "#Compile By Mr.Gaming" in list:
			Lonte.write("# Folow github : https://github.com/clayhacker-max\n"+list)
		else:
			Lonte.write(dat["garis"]+"# Obfuscate By Mr.GamingThanks To Black Coder Crush\n"+dat["github"]+dat["from"]+dat["local"]+dat["key"]+dat["date"]+dat["garis"]+list)
		Lonte.close()